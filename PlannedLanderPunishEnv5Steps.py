...
class PlannedLunarLanderPunish5Steps(gym.Env, EzPickle):
    """
    ### Description
    ...
    def __init__(
        ...
        self.observation_space = spaces.Dict({'oldActions': spaces.MultiDiscrete([4,4,4,4,4]), 'normalObs': spaces.Box(low, high)})
    ...
    def reset(self, seed: Optional[int] = None):
        ...
        if self.render_mode == "human":
            self.render()
        return self.step(np.array([0, 0]) if self.continuous else [0, 0, 0, 0, 0])[0]# , {}
    ...
    def step(self, action):
        ...
        if self.oldActionList is not None:
            for i in range(len(self.oldActionList)-1):
                #print(actionList)
                #print(self.oldActionList[1:])
                if actionList[i] != self.oldActionList[1:][i]:
                    # print("Penalty:" + str((len(self.oldActionList) - i) ** 2))
                    # print("action" + str(actionList))
                    # print("self.oldActionList" + str(self.oldActionList))
                    reward -= 1.5 * (len(self.oldActionList) - i)/len(self.oldActionList)
                    break
